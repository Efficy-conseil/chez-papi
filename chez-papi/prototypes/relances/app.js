...
function hasClientFollowup(e) {
  return !!e && !isEventPast(e) && isTruthy(e.relance_a_traiter);
}

function followupBadge(e) {
  return hasClientFollowup(e) ? '<span class="followup-badge">Nouveau message</span>' : '';
}
...
function renderDashboard() {
  ...
  const relances = actives.filter(hasClientFollowup);
  ...
  set('kpi-relances-val',    relances.length || '—');

  // Dernières demandes
  const newDemandes = appData
    .filter(e => !isEventPast(e) && (e.statut === 'Nouvelle demande' || e.statut === 'À rappeler' || hasClientFollowup(e)))
    .sort((a, b) => {
      if (hasClientFollowup(a) !== hasClientFollowup(b)) return hasClientFollowup(a) ? -1 : 1;
      return dateTimeSortValue(b.date_reception) - dateTimeSortValue(a.date_reception);
    })
    .slice(0, 6);
    ...
    if (!CONFIG.SHEETS_URL) {
      ...
      newTbody.innerHTML = newDemandes.map(e => {
        ...
        return `<tr style="cursor:pointer" onclick="openEventModal(${e._row})">
          ...
          <td>${safeText(e.nom_client)}${warningBadge}${followupBadge(e)}</td>
...
function renderPipeline() {
  ...
  el.innerHTML = PIPELINE_COLS.map(col => {
    ...
    const cards = events.map(e => {
      ...
      const followup = followupBadge(e);

      return `
      <div class="pipe-card ${hasClientFollowup(e) ? 'has-followup' : ''}" onclick="openEventModal(${e._row})">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:3px;">
          <div class="pipe-client" style="margin-bottom:0;">${safeText(e.nom_client)}</div>
          <div style="display:flex; gap:4px; align-items:center;">${followup}${warningBadge}</div>
        </div>
...
function openEventModal(rowIndex = null) {
  ...
  if (rowIndex) {
    ...
    if (existing) {
      ...
      if (followupRow) {
        const hasMessage = !!String(existing.dernier_message_client || '').trim() || hasClientFollowup(existing);
        followupRow.style.display = hasMessage ? '' : 'none';
      }
      if (followupMeta) {
        const received = existing.dernier_email_recu_le ? formatDateTimeFR(existing.dernier_email_recu_le) : 'Date non renseignée';
        const count = existing.nb_relances_client || 0;
        followupMeta.textContent = `${received} · ${count} relance${Number(count) > 1 ? 's' : ''}`;
      }
      if (followupButton) {
        followupButton.style.display = hasClientFollowup(existing) ? 'inline-flex' : 'none';
      }
    }
  } else {
...
function showKpiModal(type) {
  ...
  if (type === 'leads') {
    ...
    title.textContent = 'Messages clients';
    const evts = actives.filter(hasClientFollowup);
    evts.sort((a, b) => dateTimeSortValue(b.dernier_email_recu_le) - dateTimeSortValue(a.dernier_email_recu_le));

    thead.innerHTML = '<tr><th style="width:24%">Reçu</th><th style="width:28%">Client</th><th style="width:28%">Message</th><th style="width:20%">Statut</th></tr>';
    tbody.innerHTML = evts.length ? evts.map(e => {
      const message = String(e.dernier_message_client || '').trim();
      const shortMessage = message.length > 90 ? message.slice(0, 90) + '…' : message;
      return `<tr style="cursor:pointer" onclick="document.getElementById('kpi-modal').style.display='none'; openEventModal(${e._row})">
        <td>${safeText(formatDateTimeFR(e.dernier_email_recu_le) || 'À dét.')}</td>
        <td><strong>${safeText(e.nom_client)}</strong></td>
