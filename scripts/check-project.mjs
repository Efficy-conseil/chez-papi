...
requireText('apps-script/code.gs', '"À vérifier"');
requireText('apps-script/code.gs', 'function createUnmatchedFollowupDemand');
requireText('apps-script/code.gs', 'function mergeDemandRecords');
...
requireText('apps-script/code.gs', 'function mergeWixDuplicateDemand');
requireText('apps-script/code.gs', 'function mergeVoxistDuplicateDemand');
requireText('apps-script/code.gs', 'Les dossiers ne forment pas un doublon Wix unique par téléphone et date');
requireText('apps-script/code.gs', 'Les dossiers ne forment pas un doublon Voxist unique par téléphone et date');
requireText('apps-script/code.gs', 'matched_by: crossChannelMatches.length === 1 ? "telephone_et_date_evenement" : "wix_form_fingerprint"');
requireText('apps-script/code.gs', "clean.id_demande.indexOf('VOXIST-') === 0");
