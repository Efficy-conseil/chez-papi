...
| V09 | Audio retranscrit puis rejeté par l’IA, avec numéro appelant inconnu | Recherche prudente par téléphone, aucune ligne créée, archivage dans `Hors_Scope_Make` si aucune demande active ne correspond | Automatique + essai Make |
...
| V17 | Vocal Voxist de Madame Leclerc de Grange, appelant `06 07 03 40 37`, demandant des précisions sur un devis déjà envoyé ; l'IA retourne à tort `is_demande=false` | Le module 102 exécute le rattachement grâce au numéro appelant ; l'unique demande active portant ce numéro est enrichie, apparaît dans `Messages reçus`, sans création de ligne | Automatique + essai Make |
